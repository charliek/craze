...
| Two tags released close together disagree about `Formula/craze.rb` | Both releases push the same formula file, so whichever finishes last wins regardless of version order. **Fixed:** `concurrency` uses a single repo-wide `release` group, so a second tag's workflow queues behind the first instead of racing it | No action — but don't add a per-tag key back to that group |
