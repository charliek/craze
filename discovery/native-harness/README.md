...
| 2026-09-21/22 | H5 complete: PR 1 (#45, `b0ea4c4`) landed the harness side — the mode gate, the plan file, reminders, and the three tools (`ask_user_question`, `exit_plan_mode`, `todo_write`); PR 2 (#TBD, `feature/plan-023-h5-modes`) switched modes on in the adapter, the CLI, and the TUI, with the plan-offer eligible for a turn with no assistant text; live smoke: the first real `exit_plan_mode` on a live model behaved as designed |
