...
| Sub-agents (H6) | **Shipped (Plan 026: PR 1 #51 `f5c3cfd`, PR 2 #53 `5901e4a`, PR 3 `feature/plan-026-h6-background`, all 2026-09-24/25).** In-process: a child is a second `harness.Session` (D-54, supersedes D-10 — the child-process design this row used to describe). | The protocol's event schema; headless hosts (S4) | Met: a child's events ARE the parent session's own events — `EventSubagent`, `Event.Agent`-tagged child events — carrying the parent's own `seq`, not a private variant, because there is no separate process and no wire. What `11` asked for (children speaking the session-control schema) falls out of in-process for free. PR 2 adds `engine.Control.CancelSubagent`: non-waiting; refuses with `unknown_subagent` (stored) when the session holds no running child by that id, and with `not_accepting` (never stored) once the engine is closed. It is S2's surface, reviewed by the session-control side before commit, with SF-46 carrying its wire questions (the stop's attribution, the capability on the wire, the `tea.Cmd` move). PR 3 adds the **wake**: when a background child's result finishes idle, the session starts a turn of its own, bracketed by `EventForeignTurn{Reason: agent.ReasonSubagentWake}` — the fold picks its note by the reason, so a client that does not know it still reads today's wording. It never claims while the engine's admission fence (`agent.AdmissionFence`, now implemented by native) is up. SF-21 (a drained row refused by a foreign turn losing its place) is closed: every row-sourced turn a foreign turn refuses is restored at the queue's head; SF-47 records the residual (the ending still draws the refusal's error row, and the restore's recheck is paced but unbounded). The row band marks a background child `bg`, gated by `Capabilities.SubagentBackground` (native only) — S2 adds its wire name, `subagentBackground`, when it rebases onto this PR. After S4 a child could still become a real headless host and appear in the agent view under its parent id; that stays open, and the seam review found H6 compatible with S1c's fold (roster order is event order, the live eviction key, the wake note's wording carried in the event — all adopted in Plan 026 §3.9/§3.11). |
| Resume (H7) | **Planned (Plan 028).** `(*harness.Session).Replay(sink)` walks the path once, before the first `Run`, holding no harness lock while calling the sink; text and tool events are published synchronously, as live ones are; replayed tool ids are `<entryId>.<k>`, disjoint from live ids; a `compaction` entry replays as `Compacted{ended}` (a manual one first replays `Prompted{Text: command}`). | The journal replays UI events for every provider | **Settled by the S2 seam review** (Plan 028 "Seam review" items 2, 3, 5; plan §3.4): `Snapshot.SessionID` = the row's sessionId (bridge and attach resolve it); replay is the `EventReplay` bracket — Started follows only after it, and SD-23's "replayed" is that bracket, with no new wire flag; replay through the outbox is acceptable if every replayed event reaches the log in order inside the bracket before Started, nothing coalesces or drops, and nothing publishes under a lock the TUI's reader or an engine verb needs. The leaf→root walk stays the cross-incarnation replay authority for native (SD-23, unchanged); the new incarnation journals those replayed events, flagged. Whether earlier journals ever enrich a restored transcript is still SQ13, not H7's problem. |
| Session index (H7) | **Planned (Plan 028).** Native writes ordinary index rows through `engine.IndexOptions` (provider `native`, sessionId = the harness session id, cwd, crazeId); `Provider.Resumable()` (D-61) splits "unindexed" from `Hidden()`, so `IndexOptions.Hidden` becomes `IndexOptions.Unindexed`; a native load goes only through `build(p,row)` → `LoadSessionID`, nothing reads the store before the claim. | S1b moved index writes into the engine, **shipped** (C12: `engine.IndexOptions`, the index worker, `crazeId`) | **Settled by the S2 seam review** (Plan 028 "Seam review" items 1, 6, 9; plan §3.5): H7 takes no claim and does not reorder `resolveLoad`; the engine is built with the row's crazeId; `refuseInProcess(row provider)` runs before `EnsureCrazeID` + the claim, replacing the "a load is never in-process" skip; nothing in S2 PR 2–4 keys on `Hidden()`. S1b's engine-owned index (C12) is the seam H7 extends, not replaces: attachable live sessions (the roster) and resumable stored sessions (the index) stay different lists, and native sessions join `sessions.jsonl` for the first time. |
