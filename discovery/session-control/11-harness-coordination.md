...
| Sub-agents (H6) | **Planned (Plan 026, 2026-09-24), not executed.** In-process: a child is a second `harness.Session` (D-54, supersedes D-10 — the child-process design this row used to describe). | The protocol's event schema; headless hosts (S4) | Planned to be met trivially once PR 1 lands: a child's events will be the parent session's **own** events — `EventSubagent`, `Event.Agent`-tagged child events — carrying the parent's own `seq`, not a private variant, because there is no separate process and no wire. What `11` asked for (children speaking the session-control schema) then falls out of in-process for free; until PR 1 merges it is a design, not a fact. After S4 a child could still become a real headless host and appear in the agent view under its parent id; that stays open, and the seam review found H6 compatible with S1c's fold (roster order is event order, the live eviction key, the wake note's wording carried in the event — all adopted in Plan 026 §3.9/§3.11). |
