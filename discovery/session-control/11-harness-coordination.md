...
| Sub-agents (H6) | **Shipped (Plan 026 PR 1, #51, merged `f5c3cfd`, 2026-09-24); PR 2 adds per-child cancel.** In-process: a child is a second `harness.Session` (D-54, supersedes D-10 — the child-process design this row used to describe). | The protocol's event schema; headless hosts (S4) | Met: a child's events ARE the parent session's own events — `EventSubagent`, `Event.Agent`-tagged child events — carrying the parent's own `seq`, not a private variant, because there is no separate process and no wire. What `11` asked for (children speaking the session-control schema) falls out of in-process for free. PR 2 adds `engine.Control.CancelSubagent`: non-waiting; refuses with `unknown_subagent` (stored) when the session holds no running child by that id, and with `not_accepting` (never stored) once the engine is closed. It is S2's surface, reviewed by the session-control side before commit, with SF-46 carrying its wire questions (the stop's attribution, the capability on the wire, the `tea.Cmd` move). After S4 a child could still become a real headless host and appear in the agent view under its parent id; that stays open, and the seam review found H6 compatible with S1c's fold (roster order is event order, the live eviction key, the wake note's wording carried in the event — all adopted in Plan 026 §3.9/§3.11). |
