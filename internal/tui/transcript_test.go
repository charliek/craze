...
// The …Renders tests from here to TestEntryCapTrimsWithANoteRenders, and
// TestCancelledTurnLeavesANoteRenders below, are what a frame shows of the
// transcript facts T1a split off them (plan 024 §3.8, A13). The facts
// themselves — TestTodoToolIsNeverAdded, TestTodoStreamNotes,
// TestThoughtRunCollapsesToOneRow, TestThoughtRunShowsAMeasuredDuration,
// TestThoughtRunClosesWhenANoteLandsAfterIt, TestToolRowEndsTheThoughtRunAboveIt,
// TestEntryCapTrimsWithANote, TestOnlyTheWiresEndingClosesAStreamRun and
// TestCancelledTurnLeavesANote — are the shared model's, and are asserted in
// internal/transcript under those names, line for line: the TUI's rows are
// that model's entries now, so a copy here would assert what the package does.
...
// EventDone alone closes a run is the shared model's fact, asserted in
// internal/transcript (TestOnlyTheWiresEndingClosesAStreamRun); the property
// both were for — a turn's chunks coalesce into one run and the next turn's are
// a run of their own — is driven by TestCoalesceStreamChunks and by
// TestEnterSendsAndFollowUp.
...
// TestCancelledTurnLeavesANoteRenders holds the one assertion of
// TestCancelledTurnLeavesANote (internal/transcript) that is neither a frame nor
// a transcript fact: the TUI's own status after a cancel, which stays with the
// client.
