...
func (m Model) update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case foreignCancelledMsg:
		// The agent's own turn was cancelled and publishes no EventDone, so
		// the note is this model's to draw — but only while no craze turn is
		// current (plan 026 X44's pin): in SF-48's race the cancel landed on a
		// drained craze turn, whose synthetic ending draws its own note, and a
...
// section is cancelled instead of the agent's — and then its own synthetic
// ending draws the note, which is why the message's handler draws none while a
// craze turn is current.
