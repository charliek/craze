...
func (env Env) cacheDir(sub string, create bool) (*dir, error) {
	...
	if !filepath.IsAbs(env.Home) {
		return nil, fmt.Errorf("rundir: the home directory %q is not absolute", env.Home)
	}
	cache := filepath.Join(env.Home, cacheName)
	if _, err := os.Lstat(cache); errors.Is(err, fs.ErrNotExist) {
		if !create {
			return nil, fmt.Errorf("rundir: %s: %w", cache, fs.ErrNotExist)
		}
		if err := env.makeCache(cache); err != nil {
			return nil, err
		}
	}
	canon, err := canonical(cache)
	if err != nil {
		return nil, fmt.Errorf("rundir: resolve %s: %w", cache, err)
	}
