    ...
    def __init__(
        ...
        *,
        ...
        self.run_sh.write_text(
            "#!/bin/sh\n"
            "# generated by tests/cli/tmux_smoke.py\n"
            # The tmux server hands the pane the environment of whoever started
            # it, which may be a herdr pane or a roost tab, and craze would
            # report into that live host. Every HERDR_*/ROOST_* variable goes,
            # before a case's own env could set one on purpose.
            "unset $(env | awk -F= '/^(HERDR|ROOST)_/ {print $1}')\n"
